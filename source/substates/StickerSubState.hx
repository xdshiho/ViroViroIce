    ...
    // globalMods.push("assets/shared"); // base stickers
      ...
      if(!FileSystem.exists(modStickerDir)){
        ...
        try{
          ...
          stickers = infoObj;
          if(infoObj.getPack(STICKER_PACK) == null) FlxG.log.error('Missing pack\nSticker set ${infoObj.name} doesn\'t contain "$STICKER_PACK" pack.\n\nAll available stickers will be loaded instead.');
        }
        catch(x){
          FlxG.log.error('Couldn\'t make $STICKER_PACK\nIn "$modStickerDir":\n\n${x.message}');
        }
